...
std::vector<std::string> AnalyseDataClass::ParseString(std::string line)
{
	...
	for (unsigned int i = 0; i < line.length(); i++)// находим кол-во открываюющихс€ и закрывающихс€ скобок в строке
	{
		if (line[i] == '(')
			countOfOpenBracets++;
	...
	for (unsigned int i = indexOfOpenBracket + 1; i < indexOfCloseBracket - 1; i++)// проверка правильности написани€ внутри скобок 
